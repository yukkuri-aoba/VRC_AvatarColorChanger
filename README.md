...
このプロジェクトは以下の2つのメインブランチを使用しています：

| ブランチ | 用途 | 説明 |
|---------|------|------|
| **main** | 本番リリース | 安定版のみ。各リリースはセマンティックバージョニングでタグ付け |
| **develop** | 開発メイン | 次のリリースに向けた開発を行う。新機能・改善はここにコミット |

### 開発フロー / Development Flow

1. **develop ブランチで開発**  
   新機能や改善は `develop` ブランチでコミット

2. **テスト・動作確認**  
   dev_safe/Tests/ で単体テスト実施  
   PSD レイヤー情報との比較で検証

3. **リリース時に main へマージ**  
   リリース準備が整ったら `develop` → `main` へマージ  
   バージョンタグを付与（例：v0.2.0）
...
- 必ず**日本語**で記述してください
- 変更内容を簡潔に説明してください
- 例：`機能追加: 新しいブラシツール対応`, `修正: カラーピック精度の改善`

### 重要な規則 / Important Rules

- テクスチャ・PSD ファイルは `git` に追加しないでください（`.gitignore` 設定済み）
- バイナリファイルの管理が必要な場合は、別途 LFS の導入を検討してください
